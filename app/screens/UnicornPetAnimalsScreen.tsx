...
export default function UnicornPetAnimalsScreen() {
  const [cards, setCards] = useState(shuffle(petAnimals));
  ...
  const handleFlip = async (idx: number) => {
    ...
    try {
      const { sound: animalSound } = await Audio.Sound.createAsync(sound);
      await animalSound.playAsync();
      animalSound.unloadAsync();
    } catch (error) {
  ...
  return (
    <View style={styles.container}>
      <Text style={styles.heading}>🦄 Pet Animals Match</Text>
